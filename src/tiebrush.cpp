...
public:
    ...
    bool operator<(const SPData& b) {
        ...
        if((r->get_b()->core.flag&0x1) != (b.r->get_b()->core.flag&0x1)) return r->get_b()->core.flag&0x1; // TODO: should only be included if indexing is required
    ...
    bool operator==(const SPData& b) {
        ...
        if((r->get_b()->core.flag&0x1) != (b.r->get_b()->core.flag&0x1)) return false; // TODO: should only be included if indexing is required
...
public:
    ...
    bool add_to_offsets(uint32_t first_mate_pos,uint32_t second_mate_pos){ // store directly to offsets
        sorted_mates.insert(std::make_pair(first_mate_pos,second_mate_pos));
        return (!sorted_mates.empty() && sorted_mates.begin()->first == last_returned_pos+1); // return true if ready to write to disk
    }

    uint32_t pop_next_valid(){ // return the next valid mate position; if none available - returns 0 (since 0 can not be a valid second mate position)
    ...
    bool is_empty(){
        return (firsts.size()==0) && (sorted_mates.size()==0);
    }
...
int main(int argc, char *argv[])  {
    ...
    // TODO: inRecords.options2header(options);
...
//                    std::cout<<bam_get_qname(brec->get_b())<<std::endl;
    ...
    if(options.index){
        for(int i=0;i<dis.size();i++){
            // make sure there's no data left in the mates container
            if(!mates[i].is_empty()){
